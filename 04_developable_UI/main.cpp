...
int main(int argc, char *argv[])
{
    ...
    data.lambda = 1000;
    data.developable_threshold = 3e-2;
	...
	menu.callback_draw_custom_window = [&]()
	{
        ...
        {
            ImGui::PushItemWidth(-80);
            ImGui::DragScalar("developability", ImGuiDataType_Double, &data.developable_threshold, 3e-2, 0, 0, "%.1e");
            ImGui::PopItemWidth();
        }
        {
    ...
    const auto & draw = [&]()
    {
        ...
        {
            normal_driven_developable_single_iteration(F,U,data);
            viewer.data().clear();
            viewer.data().face_based = true;
    ...
    viewer.callback_key_pressed = [&](igl::opengl::glfw::Viewer &, unsigned int key, int mod)
    {
        switch(key)
        {
            ...
            case '}':
            {
                data.developable_threshold += 2e-6;
                break;
            }
            case '[':
            case '{':
            ...
            case ' ':
            {
                ...
                if (!state.place_constraints && state.CV.rows()>0)
                {
                    ...
                    // pre computation
                    U = V;
                    normal_driven_precomputation(V,F,1,data); // developable needs face arap
                }
