...
// to run the code, type "./normalDriven_bin spot.obj oloid.obj"
int main(int argc, char *argv[])
{
    ...
    string meshName, gaussName;
    bool useFaceArap = false;
	{   
        meshName = argv[1];
        gaussName = argv[2];

        // get gauss map positions 
        MatrixXd Vs, Vstyle;
        MatrixXi Fs;
        string styleFile = MESH_PATH + gaussName;
        igl::read_triangle_mesh(styleFile, Vstyle, Fs);
        if (Vstyle.rows() <= 0)
            cout << "read style mesh failed\n"; 
        mcf(Vstyle, Fs, Vs);

        igl::per_vertex_normals(Vs, Fs, data.gaussPos);
        igl::per_vertex_normals(Vstyle, Fs, data.desiredN);
    ...
    const auto & draw = [&]()
    {
        ...
        {
            normal_driven_gauss_single_iteration(F,U,data);
            viewer.data().clear();
            viewer.data().face_based = true;
